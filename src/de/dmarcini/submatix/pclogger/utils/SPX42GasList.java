...
//@formatter:on
  ...
  {
    if (number >= GASCOUNT) { return 0; }
    if (bailout[number] > 0) { return (bailout[number]); }
    return (0);
  }
  ...
  {
    if (number < GASCOUNT || currgas != number) { return (0); }
    return (1);
  }
  ...
  {
    if (number >= GASCOUNT) { return false; }
    if (toSet)
    {
      bailout[number] = 3;
    }
  ...
  public boolean setGas(String fromSpx)
  {
    ...
    init[vals[0]] = true;
    lg.info(String.format("gas: NR %d: %d%% O2, %d%% HE, %d%% N2, DIL: %d, Bailout: %d...", vals[0], 100 - n2[vals[0]] - he[vals[0]], he[vals[0]], n2[vals[0]],
