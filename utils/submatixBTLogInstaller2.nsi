...
OutFile "SubmatixBTLog-1.0a-Setup.exe"
...
Section "MAIN" SEC01
  ...
  CreateShortCut "$SMPROGRAMS\submatixBTLog\submatixBTLog Ver. 1.0a.lnk" "$INSTDIR\submatix_start.bat" icon.file "$INSTDIR\progicon.ico"
  CreateShortCut "$SMPROGRAMS\submatixBTLog\submatixBTLog Ver. 1.0a DEBUG.lnk" "$INSTDIR\submatix_start.bat" icon_file "$INSTDIR\progicon.ico"
SectionEnd
...
Section Uninstall
  ...
  Delete "$INSTDIR\lib\librxtxParallel.so"
  Delete "$INSTDIR\lib\librxtxSerial.jnilib"
  Delete "$INSTDIR\lib\librxtxSerial.so"
