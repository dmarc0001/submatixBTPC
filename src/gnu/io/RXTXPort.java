...
final public class RXTXPort extends SerialPort
{
  ...
  public RXTXPort( String name ) throws PortInUseException
  {
    ...
    /*
     * commapi/javadocs/API_users_guide.html specifies that whenever an application tries to open a port in use by another application the PortInUseException will be thrown
     * 
   ...
   * 
   * This is important for applications that flicker the Monitor thread while keeping the port open. In worst case test cases this loops once or twice every time.
   */
  ...
  class SerialOutputStream extends OutputStream
  {
    ...
    public void write( byte b[], int off, int len ) throws IOException
    {
      ...
      try
      {
        ...
        if( debug_write ) z.reportln( "Leaving RXTXPort:SerialOutputStream:write(" + send.length + " " + off + " " + len + " " + ") " /* + new String(send) */);
      }
      finally
      {
        synchronized( IOLockedMutex )
  ...
  class SerialInputStream extends InputStream
  {
     ...
     * @see java.io.InputStream
     * 
     *      timeout threshold Behavior ------------------------------------------------------------------------ 0 0 blocks until 1 byte is available timeout > 0, threshold = 0,
     *      blocks until timeout occurs, returns -1 on timeout >0 >0 blocks until timeout, returns - 1 on timeout, magnitude of threshold doesn't play a role. 0 >0 Blocks until 1
     ...
     * @throws IOException
     * 
     *           timeout threshold Behavior ------------------------------------------------------------------------ 0 0 blocks until 1 byte is available >0 0 blocks until timeout
     *           occurs, returns 0 on timeout >0 >0 blocks until timeout or reads threshold bytes, returns 0 on timeout 0 >0 blocks until reads threshold bytes
     */
    @Override
     ...
     * 
     *           timeout threshold Behavior ------------------------------------------------------------------------ 0 0 blocks until 1 byte is available >0 0 blocks until timeout
     *           occurs, returns 0 on timeout >0 >0 blocks until timeout or reads threshold bytes, returns 0 on timeout 0 >0 blocks until either threshold # of bytes or len bytes,
     *           whichever was lower.
     */
    @Override
    public synchronized int read( byte b[], int off, int len ) throws IOException
    {
      ...
      {
        /*
         * If threshold is disabled, read should return as soon as data are available (up to the amount of available bytes in order to avoid blocking) Read may return earlier
         * depending of the receive time out.
         */
      ...
      try
      {
        ...
        if( debug_read_results ) z.reportln( "RXTXPort:SerialInputStream:read(" + b.length + " " + off + " " + len + ") returned " + result + " bytes" /* + new String(b) */);
        return( result );
      }
      finally
      {
     ...
     * 
     *           if the read encounters the two bytes, it will return and the array will contain the terminator. Otherwise read behavior should be the same as read( b[], off, len
